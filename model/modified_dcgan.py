...
def generator_model(input_size, # input size that was set to create the latent space (random numbers)
    ...
    generator.add(Dense(4*4*512, input_shape = (input_size, ), kernel_initializer=RandomNormal(stddev=kernel_std))) # units were referred from the paper
    generator.add(Reshape(target_shape=(4, 4, 512))) 
    generator.add(BatchNormalization()) 
    generator.add(LeakyReLU(alpha)) 
    
    #second block 
    generator.add(Conv2DTranspose(256, kernel_size=5, strides=2, padding='same',kernel_initializer=RandomNormal(stddev=kernel_std)))
    generator.add(BatchNormalization())
    generator.add(LeakyReLU(alpha))

    #fourth block 
    generator.add(Conv2DTranspose(128, kernel_size=5, strides=2, padding='same', kernel_initializer=RandomNormal(stddev=kernel_std)))
    generator.add(BatchNormalization())
    generator.add(LeakyReLU(alpha))

    #final block 
    generator.add(Conv2DTranspose(3, kernel_size=5, strides=2, padding='same', kernel_initializer=RandomNormal(stddev=kernel_std)))
...
def discriminator_model(alpha=0.2, # alpha value for the leaky relu 
    ...
    discriminator.add(Conv2D(64, kernel_size=5, strides=2, padding='same',kernel_initializer=RandomNormal(stddev=kernel_std), input_shape=(32, 32, 3)))
    discriminator.add(LeakyReLU(alpha))
    discriminator.add(Conv2D(128, kernel_size=5, strides=2, padding='same', 
               kernel_initializer=RandomNormal(stddev=kernel_std)))
    ...
    discriminator.add(Dense(1, kernel_initializer=RandomNormal(stddev=kernel_std)))
